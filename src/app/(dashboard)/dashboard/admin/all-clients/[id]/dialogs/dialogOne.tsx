...
export default function DialogOne(
  ...
  const [image, setImage] = useState<Partial<TUploadedFile> | null>(
    payload?.user?.profileImage ? payload?.user?.profileImage : null
  );
  ...
  const formSubmit: SubmitHandler<ClientUpdateFormValues> = async (data) => {
    ...
    try {
      const project = await updateClientData({ data: clientData, userId: payload?.userId });
          ...
          >
            ...
            <Stack alignItems='center' gap='1rem' justifyContent='end'>
              ...
              <Button
                ...
                disabled={isLoading}
              >
                {isLoading ? 'Loading...' : isError ? 'Error' : 'Update Client'}
              </Button>
            </Stack>
